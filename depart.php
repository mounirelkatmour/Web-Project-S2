...
<head>
    ...
    <link rel="stylesheet" href="depart.css" />
    <title>Departements - Gestagiaire.ma</title>
</head>
<body>
    ...
    <div class="navbar">
        ...
        <div class="useless" style="flex-grow: 1"></div>
        ...
        <a href="adddepart.php" class="navEl" id="adddepart">Add department</a>

      <button class="Logout" onclick="location.href='logout.php'">
        Log Out
      </button>
    </div>
    <p id="listTxt">List of departments :</p>
    <table border="1">
        <tr>
        ...
        if ($result->num_rows > 0) {
            while($row = $result->fetch_assoc()) {
                echo "<tr>";
                echo "<td>" . $row["id_depart"] . "</td>";
                echo "<td>" . $row["name_depart"] . "</td>";
                echo "<td>" .  adminId2Name($row["id_admin"])  . "</td>";
                echo "<td>";
                echo "<a href='editdepart.php?name_depart=" . $row["id_depart"] . "'><button id='btn1' class='btn'>Edit</button></a>";
                echo "<button id='btn2' class='btn' onclick='confirmDelete(" . $row["id_depart"] . ")'>Remove</button>";
                echo "</td>";
                echo "</tr>";
            }
        } else {
            echo "<tr><td colspan='5'>No departements found</td></tr>";
        }
        ?>
    </table>
...
function confirmDelete(departId) {
    if (confirm("Are you sure you want to delete this departement?")) {
        window.location.href = "removedepart.php?id=" + departId;
    } else {
...
var x = 0;
    ...
    document.getElementById("newDiv").addEventListener("click", function () {
      ...
      var navElements = document.querySelectorAll(
        "#logo, #home, #departments, #interns, #internships,#adddepart, .Logout"
      );
